...
const mockPayment = async (req, res, next) => {
  try {
    const payment = await paymentService.mockPay(req.user, req.body.orderId);
...
const listMyPayments = async (req, res, next) => {
  try {
    const result = await paymentService.listPayments({ ...req.user, role: 'user' }, req.query);
...
const updateStatus = async (req, res, next) => {
  try {
    ...
    return response.success(res, { payment }, 'Payment status updated successfully');
  } catch (err) { return next(err); }
};
...
module.exports = { mockPayment, listMyPayments, listPayments, getPayment, updateStatus, refund };