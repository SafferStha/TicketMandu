...
export const paymentsAPI = {
  mockPay: async (orderId) => {
    const response = await API.post('/payments/mock', { orderId });
    return unwrapResource(response, 'payment');
  },
  getMy: async (params) => {
    const response = await API.get('/payments/my', { params: cleanParams(params) });
    return { payments: unwrapList(response), pagination: unwrapPagination(response) };
  },
  getAll: async (params) => {
