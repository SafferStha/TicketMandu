...
import { events, categories, recentSearches } from '../data/mockData';
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    if (initialQ) { setQuery(initialQ); setSubmitted(true); }
  }, [initialQ]);

  const allCats = [{ id: 'all', name: 'All', icon: '✨' }, ...popCategories];

  const results = events.filter(e => {
    const matchQ = !submitted || !query
      ? true
      : e.name.toLowerCase().includes(query.toLowerCase()) ||
        e.category.toLowerCase().includes(query.toLowerCase()) ||
        e.venue.toLowerCase().includes(query.toLowerCase());
    const matchCat = selectedCat === 'all' || e.category.toLowerCase() === selectedCat;
        ...
        {!submitted ? (
            ...
            <section style={styles.section}>
              ...
              <div style={styles.recentList}>
                {recentSearches.map((term, i) => (
                  <button key={i} style={styles.recentItem} onClick={() => handleRecentClick(term)}>
                    <ClockIcon />
            ...
            <section style={styles.section}>
              ...
              <div style={styles.catGrid}>
                {popCategories.map(cat => (
                  <button key={cat.id} style={styles.catCard} onClick={() => handleCategoryClick(cat.id)}>
          ...
          <>
            ...
            <div style={styles.filterRow}>
              <CategoryChips categories={allCats} selected={selectedCat} onSelect={setSelectedCat} />
            </div>
                ...
                {results.length > 0
                  ...
                  : (
                    <div style={styles.noResults}>
                      ...
                      <p style={styles.noResultsSub}>Try different keywords or categories</p>
                    </div>
                  )
                }
              </div>
